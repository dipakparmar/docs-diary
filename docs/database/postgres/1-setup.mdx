---
id: setup
title: PostgresSQL Setup 
sidebar_label: Setup
keywords: [PostgresSQL, libpq, psql, pg_dump, postgres, install postgres, install postgress client, install psql, install pgdump]
...
To install PostgresSQL using homebrew, run the following command:
...
:::warning
You do not need to install `libpq` if you already have PostgresSQL installed using above command. Ony install libpq if you do not have postgresql installed and want to use psql and pg_dump clients.
:::
...
This will give you access to utilities such as __psql__ and __pg_dump__ without having to install the full Postgres package. 
...
but then __you would not be able to install the Postgres package later__.

## Managing the PostgresSQL
...
2. [Homebrew - PostgresSQL wiki](https://wiki.postgresql.org/wiki/Homebrew)
